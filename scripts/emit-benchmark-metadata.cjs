...
const HASHED_REQUIRED_FILES = Object.freeze([
  ...
  // All 37 FIXED_TEST_FILE_INVENTORY suites are cited (sha256) so the
  ...
  'src/formats/projectJson.ts',
  'src/formats/blueprintPng.ts',
  'src/aero/finStructure.ts',
  'src/aero/stabilityBreakdown.ts',
  'src/formats/rktExport.ts',
  'src/formats/stepExport.ts',
  'src/formats/stlExport.ts',
  'src/propulsion/curveEditing.ts',
  'src/propulsion/gibbsEquilibrium.ts',
  'src/propulsion/thrustcurveApi.ts',
  'src/sim/motorVariance.ts',
  'src/sim/waiverContainment.ts',
  'src/sim/windProfile.ts',
