/**
 * Monte Carlo web-worker entrypoint (E1/Q12: Vite module worker).
 *
...
export interface MonteCarloChunkRequest {
  ...
  nRuns: number;
  seed: number;
  /** Absolute start run index (inclusive) of this chunk. */
  runStart: number;
  /** Absolute end run index (exclusive) of this chunk. */
  runEnd: number;
}
  ...
  switch (message.type) {
    ...
    case 'montecarlo:run': {
      ...
      const result = runMonteCarloChunk(
        ...
        message.request.runStart,
        message.request.runEnd,
      );
