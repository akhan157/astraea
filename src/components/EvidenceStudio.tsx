 ...
 * calibration, and recovery-packing/charge sizing in one panel.
 *
 * Three self-contained cards, following the FlightSimulationTab surface
 ...
 *   3. Recovery — bay sizing (volume, packed density, pack advisory badge)
...
/** Card 3 — bay packing + shear-pin/BP charge sizing (live readouts). */
function RecoveryCard(): React.JSX.Element {
  ...
  const bayLengthM = toNumber(bayLengthText);
  const bayDiameterM = toNumber(bayDiameterText);
  const chuteMassG = toNumber(chuteMassText);
  const pinCount = toNumber(pinCountText);

  const volumeM3 = bayLengthM !== null && bayDiameterM !== null && bayLengthM > 0 && bayDiameterM > 0
    ? bayVolume(bayLengthM, bayDiameterM)
    : null;
  const densityGcm3 = chuteMassG !== null && volumeM3 !== null && chuteMassG > 0
    ? packedDensity(chuteMassG, volumeM3)
    : null;
  ...
  const targetPressurePa = pinCount !== null && bayDiameterM !== null && pinCount > 0 && bayDiameterM > 0
    ? targetPressure(pinCount, pinForceN, bayDiameterM)
    : null;
  ...
  return (
    <Card
      title="Recovery"
      kicker="Bay packing density + shear-pin / black-powder charge sizing"
